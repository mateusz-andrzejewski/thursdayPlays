export class RenderTeams {
  constructor(teams) {
    this.introductionTxt =
      "Hej w czwartek gramy na hali mosir od 18:00 do <UZUPEŁNIJ>. Koszt: <UZUPEŁNIJ> zł. Proszę o wcześniejszy przelew BLIK na tel: 793640196. Składy: ";
    this.teams = teams;
  ...
  teamSkillRating(team) {
    ...
    const avg = (
      (sRofTeam.reduce((prev, next) => +prev + +next) / (team.length * 5)) *
      100
    ).toFixed(0);
    return `${avg.toString()}`;
  }
  ...
  makeResultTxt() {
    ...
    for (const team of this.teams) {
      const arr = [];
      const teamNum = `Team ${
        this.teams.indexOf(team) + 1
      } ${this.getTshirtColor(this.teams.indexOf(team) + 1)} `;
    ...
    const wholeTxt = JSON.stringify(txtTeams)
      .replace(/"/gi, "")
      .replace(/,/gi, ", ")
      .replace(/\[|\]/g, "");
    this.wholeTxt = this.introductionTxt + "  " + wholeTxt;
  }
  copyTxt() {
  ...
  getTshirtColor(teamNumber) {
    switch (teamNumber) {
      case 1:
        return " -> KOSZULKA CZARNA";
      case 2:
        return " -> KOSZULKA BIAŁA";
      case 3:
        return " -> KOSZULKA INNA NIŻ BIAŁA I CZARNA";
    }
  }
}
